...
### 11. Control chọn dữ liệu từ Database → BẮT BUỘC `LookUpEdit`

> **Mọi control "chọn 1 giá trị" mà nguồn (`ItemsSource`) được LOAD TỪ DATABASE
> đều phải dùng `dxg:LookUpEdit`** — KHÔNG dùng `dxe:ComboBoxEdit`.
> Combo (`ComboBoxEdit`) chỉ dành cho **danh sách enum/option tĩnh** khai báo cứng
> trong ViewModel (VD: `["web","mobile","wpf"]`, severity, view type…).

**Cách phân biệt:**

| Nguồn `ItemsSource` | Control đúng |
|---|---|
| Query DB / service (`await _service.GetXxxAsync(...)`), danh sách bảng, cột, lookup code, form code, danh mục… | `dxg:LookUpEdit` |
| List hằng gõ tay trong VM (`= ["A","B"]`), enum cố định | `dxe:ComboBoxEdit` |

**Sửa thuộc tính chỉ ở MỘT nơi:** toàn bộ hành vi + visual của `LookUpEdit`
(chiều cao, gõ-để-lọc `Contains`, popup lưới đa cột `LookUpGridPopup`, auto-filter,
`AutoPopulateColumns`…) đã được khai báo **một lần** trong style implicit tại
[`Themes/Controls.xaml`](src/frontend/ConfigStudio.WPF.UI/src/ConfigStudio.WPF.UI/Themes/Controls.xaml)
(`<Style TargetType="{x:Type dxg:LookUpEdit}">`). Muốn đổi hành vi cho **mọi** lookup
→ sửa đúng style đó, KHÔNG set lại thuộc tính tại từng View.

**Trong View chỉ khai báo dữ liệu, không khai báo style:**

```xml
<!-- ĐÚNG — lookup nguồn DB: chỉ ItemsSource / DisplayMember / EditValue -->
<dxg:LookUpEdit ItemsSource="{Binding MasterColumns}"
                DisplayMember="..." ValueMember="..."
                EditValue="{Binding EditMasterKeyColumn, UpdateSourceTrigger=PropertyChanged}"
                NullText="Id" />

<!-- SAI — combo cho dữ liệu DB, lại còn set IsTextEditable/IncrementalFiltering tại chỗ -->
<dxe:ComboBoxEdit ItemsSource="{Binding MasterColumns}" IsTextEditable="True" ... />
```

- KHÔNG set `IsTextEditable` / `ImmediatePopup` / `IncrementalFiltering` /
  `FilterCondition` / `AutoPopulateColumns` … tại View — đã có sẵn trong style chung.
- Combo nhúng cột lưới (`dxe:ComboBoxEditSettings`) cho dữ liệu DB: tương tự ưu tiên
  `LookUpEditSettings`; còn enum tĩnh thì giữ `ComboBoxEditSettings`.

**Hai dạng nguồn — chọn đúng style (đều khai báo trong `Controls.xaml`):**

| Nguồn `ItemsSource` | Cách dùng | Lý do |
|---|---|---|
| List **OBJECT** (có property: `Tables` có `DisplayText`…) | `LookUpEdit` **không** cần `Style` (đã có style implicit) + khai báo `DisplayMember`/`ValueMember`/`SelectedItem` (hoặc `EditValue`) | Popup lưới tự sinh cột bằng `AutoPopulateColumns` |
| List **SCALAR** (`List<string>` / `ObservableCollection<string>`: cột bảng, mã lookup, form code…) | `LookUpEdit Style="{StaticResource ScalarLookUpEditStyle}"` + `ItemsSource` + `EditValue` | DevExpress **không** auto-sinh cột cho kiểu nguyên thủy → style scalar tắt `AutoPopulateColumns` và dùng popup 1 cột `Binding="{Binding}"` (đã định nghĩa sẵn là `LookUpScalarPopup`). Quên `Style` này → popup ra **hàng trống** |

> Verified DevExpress WPF 25.2: `GridColumn.Binding` (BindingBase) tồn tại; `AutoPopulateColumns`
> KHÔNG sinh cột cho `List<string>`. Đừng đoán — đã kiểm bằng reflection.

### 12. Lưới dữ liệu (`dxg:GridControl` / `dxg:TableView`) — 3 nguyên tắc BẮT BUỘC

> **MỌI lưới trong app đều phải có đủ 3 thứ sau. Không có ngoại lệ, không khai báo lại ở View.**

| # | Nguyên tắc | Người dùng thấy gì |
|---|---|---|
| 1 | **Lọc kiểu "chứa"** trên mọi cột | Hàng ô lọc ngay dưới header; gõ tới đâu lọc tới đó; cột text lọc **Contains**, không phân biệt hoa/thường |
| 2 | **Nhớ format lưới** | Độ rộng / thứ tự / ẩn-hiện / sắp xếp cột được lưu, mở lại màn hình vẫn y nguyên |
| 3 | **Tự sắp lưới** | Kéo giãn độ rộng cột + kéo đổi vị trí cột |

**Cài ở đâu — sửa đúng 1 nơi:**

- Style implicit `dxg:TableView` + `dxg:GridColumn` trong
  [`Themes/Controls.xaml`](src/frontend/ConfigStudio.WPF.UI/src/ConfigStudio.WPF.UI/Themes/Controls.xaml):
  `ShowAutoFilterRow`, `AllowColumnFiltering`, `AllowColumnMoving`, `AllowResizing`,
  `ImmediateUpdateAutoFilter`, và `GridLayoutBehavior.Persist=True`.
- [`Infrastructure/Behaviors/GridLayoutBehavior.cs`](src/frontend/ConfigStudio.WPF.UI/src/ConfigStudio.WPF.UI/Infrastructure/Behaviors/GridLayoutBehavior.cs):
  lưu/phục hồi format ra file XML local (`%LOCALAPPDATA%\ICare247\ConfigStudio\GridLayouts`,
  **không đụng DB**) + ép chính sách lọc.

**Trong View chỉ khai báo cột + dữ liệu** — TUYỆT ĐỐI không set lại `ShowAutoFilterRow` /
`AllowColumnMoving` / `AllowResizing` / `AutoFilterCondition` tại chỗ. Muốn đổi hành vi cho
**mọi** lưới → sửa style implicit, không sửa từng màn.

**Quyết định đã chốt (đừng đảo ngược khi refactor):**

- **`AutoFilterCondition` KHÔNG đặt trong style** — gán theo `FieldType` trong `ApplyFilterPolicy`:
  cột text → `Contains`; cột số / ngày / bool → `Default` (so sánh bằng). `Contains` trên kiểu
  không phải chuỗi là vô nghĩa.
- **Bộ lọc KHÔNG được nhớ giữa các lần mở màn hình** — chỉ nhớ *format cột*. Mở lại lưới luôn
  thấy ĐỦ dữ liệu, tránh người dùng tưởng "mất dữ liệu" vì bộ lọc cũ còn sót. Nạp lại dữ liệu
  (`ItemsSourceChanged`) thì **giữ nguyên** bộ lọc người dùng đang gõ.
- **Phải ép lại chính sách lọc SAU `RestoreLayoutFromXml`** — file layout lưu cả tuỳ chọn view;
  file sinh ra trước khi bật auto-filter mang `ShowAutoFilterRow=False` và sẽ **đè** setter trong
  `Controls.xaml` → hàng lọc "biến mất". Đây là gotcha đã gặp, không phải bug DevExpress.
- **Popup lưới của `LookUpEdit`** đặt `Style="{x:Null}"` nên **không** dính implicit style
  `TableView` → không bị lưu layout, không có hàng auto-filter (nó có cơ chế gõ-để-lọc riêng).
  Giữ nguyên `{x:Null}` khi sửa popup.
- Thêm `TreeListView` sau này → áp cùng 3 nguyên tắc (`TreeListView` cũng có `ShowAutoFilterRow` /
  `AllowColumnMoving` / `AllowResizing`); hiện app chưa dùng TreeList.

> Verified DevExpress WPF 25.2 bằng reflection: `TableView.ShowAutoFilterRow` /
> `AllowColumnMoving` / `AllowColumnFiltering` / `AllowResizing` đều là DP;
> `GridColumn.AutoFilterCondition` là enum `{Like, Equals, Contains, Default}`;
> `GridColumn.ImmediateUpdateAutoFilter`, `FieldType`, `AutoFilterValue` tồn tại;
> `ColumnsPopulated` đã **obsolete** → dùng `AutoGeneratedColumns`.
