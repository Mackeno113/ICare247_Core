...
**Multi-Trigger (Migration 068):** ô **🔄 "Tự reload theo nhiều field cha"** lưu cột
`Ui_Field_Lookup.Reload_Trigger_Fields` (CSV FieldCode). Runtime **hợp** danh sách này với
`@param` Filter SQL + `ReloadTriggerField` đơn → reload khi bất kỳ field cha nào đổi. Dùng cho
TVF/Full SQL hoặc field cha **không** có trong Filter SQL.

**Vùng cũ — KHÔNG dùng:**
- Khung **⚡ Tham số từ field khác** (`filterParams`) → runtime không tiêu thụ.

→ Cascade chuẩn table-mode = **chỉ viết Filter SQL** với `@FieldCodeCha`; nhiều cha ngoài filter → Multi-Trigger. Xem thêm
