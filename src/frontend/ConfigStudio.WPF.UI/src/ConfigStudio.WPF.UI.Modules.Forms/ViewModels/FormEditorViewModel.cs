...
public sealed class FormEditorViewModel : ViewModelBase, INavigationAware
{
        ...
        _dialogService.ShowDialog(
            ...
            result =>
            {
                ...
                if (newNodes.Count > 0 && _fieldDataService is not null && _appConfig is not null)
                    _ = PersistAutoGeneratedFieldsAsync(targetSection, newNodes);
            });
    ...
    /// 1. Lấy TableId → lookup ColumnId từ Sys_Column theo ColumnCode.
    /// 2. INSERT từng field vào Ui_Field, cập nhật node.Id với Field_Id thật.
    /// 3. Lưu LabelKey vào Sys_Resource (vi = ColumnName, en = ColumnName fallback).
    /// </summary>
        ...
        IReadOnlyList<FormTreeNode> nodes)
    {
        ...
        try
        {
            ...
            // ── Lấy TableId và danh sách Sys_Column ──────────────
            var tableId = await _fieldDataService.GetTableIdByFormAsync(
                FormId, _appConfig.TenantId, cts.Token);

            IReadOnlyList<ColumnInfoRecord> sysColumns = tableId > 0
                ? await _fieldDataService.GetColumnsByTableAsync(tableId, cts.Token)
                : [];

            // Index theo ColumnCode (case-insensitive) để lookup nhanh
            var colDict = sysColumns.ToDictionary(
                c => c.ColumnCode,
                c => c.ColumnId,
                StringComparer.OrdinalIgnoreCase);
            ...
            {
                colDict.TryGetValue(node.Code, out var columnId);
                ...
                // Lưu Sys_Resource LabelKey — vi = ColumnName (fallback đơn giản)
                if (_i18nService is not null && fieldId > 0 && !string.IsNullOrEmpty(node.TitleKey))
                    await _i18nService.SaveResourceAsync(
                        node.TitleKey, "vi", node.Code, cts.Token);
            }
