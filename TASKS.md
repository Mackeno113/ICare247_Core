...
- [x] **AUTHZ-SEC-2** — `db/046` thêm `DoiTuong`+`LoaiDoiTuong` vào `HT_ChucNang` (link form/view); `IPermissionService.HasPermissionForTargetAsync` (**enforce-if-mapped**) + `[RequirePermissionForTarget]`; gắn `MasterData`(Form: Xem/Thêm/Sửa/Xóa) · `View`(View: Xem) · `Runtime`(Form: Xem). Build xanh. ⏳ chạy `db/046` + set DoiTuong cho màn cần khóa.
