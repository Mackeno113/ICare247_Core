...
Kế hoạch đầy đủ → `C:\Users\Mackeno_01\.claude\plans\inherited-floating-shannon.md` (Feature A/B/C).
Commit: `4c656a7` (C) → `6d467fe` (A) → `44ed677` (B). **Build verify /finish-task**: backend
0 `error CS` (fail còn lại chỉ file-lock API đang chạy) · Web 0W/0E · ConfigStudio WPF 0W/0E ·
`dotnet test ICare247.Application.Tests` 145/145 pass.

**⚠️ Trước khi restart API trên môi trường thật — chạy đúng thứ tự:** `db/085` → `086` → `087` →
`088`, nếu không mọi màn `/view/...` (không riêng TreeList) lỗi "Invalid column name" (xem lý do
từng Feature bên dưới — `ViewRepository.GetByCodeAsync` là hub chung).

- [x] **Feature C — TreeList kéo-thả sắp xếp (ADR-027)**: `db/085` proc generic `sp_RecomputeTreeOrder`
  (Data DB, đệ quy CTE tính Cap/ThuTuCay/DuongDanCay theo @TableName+@ParentColumn bất kỳ) + `db/086`
  `Ui_View.Allow_Reorder` (Config DB) · Backend: `IViewRepository.ReorderAsync` (chặn cycle qua CTE
  hậu duệ, renumber ThuTu 10/20/30 tập anh em mới, EXEC proc) + `POST /api/v1/views/{code}/reorder`
  (`ReorderTreeItemCommand`, quyền `Sua`) · Web: `ViewApiService.ReorderAsync` + `DataView.razor`
  (`AllowDragRows`/`AllowedDropTarget.Internal`/`DropTargetMode.BetweenRows`/`ItemsDropped` — API xác
  nhận có thật qua reflection `DevExpress.Blazor.v25.2.dll`, không đoán) → `OnReorder` bubble lên
  `ViewPage.razor` gọi API rồi LUÔN reload (DxTreeList không tự cập nhật Data theo kéo-thả) · WPF:
  checkbox "Allow_Reorder" tab Cây (`ViewManagerView.xaml`/`ViewManagerViewModel.cs`/`ViewDataService.cs`/
  `ViewRecord`/`ViewUpsertRequest`). Build 4 lớp (Api+Application+Infra qua build riêng do API đang
  chạy khóa file — 0 error CS; Web 0W/0E; WPF 0W/0E).
  **⚠️ GitNexus risk CRITICAL (24 execution flow)** — lý do: `ViewRepository.GetByCodeAsync` là hub mọi
  màn `/view/...` đều đi qua, thêm cột `Allow_Reorder` vào SELECT nghĩa là **BẮT BUỘC chạy db/086
  trước khi deploy backend mới**, nếu không mọi màn View (không riêng TreeList) lỗi "Invalid column
...
- [x] PICKER-P4 (2026-07-16): db/083 Ui_Lookup_Template + Ui_Field_Lookup.Template_Code/Param_Map + seed 3 mẫu (TPL_CONG_TY qua fnt_CongTyTheoQuyen db/084 — custom_sql bị chặn chuỗi con "IsDeleted"; TPL_NHAN_VIEN đợi NS_) · engine resolve template + Param_Map (field/@token/hằng số) + token tự resolve mọi @param thiếu qua IContextParamResolver + **cache key hash SAU khi bind đủ tham số** (token theo user phải vào key) + merge Param_Map→Reload_Trigger_Fields + fallback legacy khi tenant chưa migrate · ConfigSync descriptor · ConfigStudio WPF section "Mẫu lookup dùng chung" (combo + lưới map, IO phòng thủ). Build BE + WPF 0W/0E. Đã chạy 082/083/084 vào DB (SSMS, 2026-07-18); sau đó đổi tên fn_→fnt_CongTyTheoQuyen theo quy tắc mới; **đã re-run 083+084** (SSMS, 2026-07-18, xác nhận bởi user) — hàm `fnt_CongTyTheoQuyen` + dòng seed TPL_CONG_TY đã đúng tên mới. CHƯA smoke runtime (dời sau màn Phòng ban); hover-help topic cho section mới để sau
- [x] PICKER-P3b / AddressBox editor (2026-07-19): nối IcAddressBlock (P3) vào **form engine no-code** — EditorType `AddressBox` (Editor_Type→`NormalizeFieldType`="address" ở 3 nơi: MasterDataForm/FormRunner/LookupQueryService) → `FieldRenderer` case "address" → **AddressRenderer** host IcAddressBlock. Cơ chế **2-field** (vì backend `BuildColumnParams` chỉ ghi cột field đã khai): field NEO map cột xã/phường (int) render cả khối + bắn OnChange cho companion; field text (địa chỉ) là companion `IsHiddenByComposite` (ẩn-render nhưng-vẫn-lưu) qua `CompositeFieldHelper.MarkAddressCompanions` gọi ở cả 2 host. Tỉnh nội bộ khối, không lưu. ConfigStudio: `AvailableEditorTypes`+AddressBox, `AddressBoxPropsPanel` (chọn field text), ControlProps `addressTextField`, guide+reset. **Backend KHÔNG đổi** (tái dùng PickersController). Fix: State.Value khi Sửa về JsonElement → `ToLong` unwrap để tỉnh/xã resolve (địa chỉ text vẫn hiện vì ToString ra chuỗi). Build Web+WPF **0W/0E**; runtime CHƯA smoke (user rebuild+hard reload). Doc `docs/huong-dan-wpf/cau-hinh-address-box.md`. Còn: xóa field Tỉnh riêng trên form Công ty (trùng)
- [x] Fix SpinEdit ViewManager (2026-07-19): `Expand_Level` + `Page_Size` thêm `IsFloatValue="False"` — hết lỗi *"Value 'X' could not be converted"* (SpinEdit chạy float mode → decimal "2." không convert được về int/int?). Theo pattern có sẵn `SysLookupManagerView`. Build WPF 0W/0E
...
- [~] B4.2 nhóm 1/4 — Cb* (8 prop + 4 option list) + Sys_Lookup tĩnh (LookupCode/AvailableLookupCodes/LookupPreviewItems + 2 loader) ĐÃ SỞ HỮU trong FkLookupConfigVm; root truy cập qua FkLookup.X, side-effect rebuild qua hook internal NotifyLookupPropChanged (guard cờ đang-rebuild giữ nguyên); restore-từ-JSON + reset dời theo (RestoreComboPropsFromJson/ResetComboAndLookupState). Root 2.740 → 2.597 dòng. Build 0W/0E
- [x] B4.2 nhóm 2 (2026-07-18) — EditBox/Tree/AddNew (EditBoxMode/IsCodeAndNameMode, CodeField, ImportGlobalCode, DropDown W/H, ReloadTriggerField, ParentColumn, TreeSelectableLevel, AllowAddNew, AddFormCode, AvailableFormCodes + LoadFormCodesAsync + 2 option list) SỞ HỮU trong FkLookupConfigVm theo đúng công thức nhóm 1 (side-effect: IsDirty qua root, ReloadTriggerField→root.RecomputeCascadeWarnings internal, ParentColumn→NotifyLookupPropChanged; restore/raise/reset = 4 method internal RestoreLookupDbConfig/RaiseLookupDbProps/RestoreImportGlobalCode/ResetLookupDbState — giữ nguyên thứ tự guard `_isRebuildingProps` + quirk Clear-không-raise). Kèm **fix bug nhóm 1 để lại:** section RadioGroup Sys_Lookup trong FieldConfigView.xaml vẫn bind LookupCode/AvailableLookupCodes/LookupPreviewItems vào root (đã dời sang con) → binding gãy âm thầm; sửa StackPanel DataContext=FkLookup. Root 2.927 → 2.769 dòng (số "2.597" ghi ở nhóm 1 là đếm sai — số thật tại HEAD trước bước này là 2.927); FkLookupConfigVm 302 → 500. Build module 0W/0E
- [x] B4.2 nhóm 3 (2026-07-18) — nguồn FK (QueryMode + Fk* 9 prop + 5 collection FkPopupColumns/FkFilterParams/FkFunctionParams/ReloadOnChangeFields/DataSourceConditions + ReloadOnChangeInput + 4 option list + 13 command) SỞ HỮU trong FkLookupConfigVm; **lambda `+=` → handler đặt tên** (OnFkColumnPropertyChanged auto-gen captionKey, OnFkItemPropertyChanged rebuild-only, WireFkColumnHandlers/WireRebuildOnChange). Guard `_isRebuildingProps` vẫn ở root, VM con đọc qua `root.IsRebuildingProps`; RebuildControlPropsJson + RecomputeCascadeWarnings thành internal cho con gọi. Clear/reset qua ResetFkSourceState + RaiseFkSourceProps (giữ đúng tập raise cũ). **XAML:** xóa 2 khối legacy Collapsed (FK Lookup Config ~347 dòng + Kiểm tra&Diễn giải ~34 dòng); 2 section active còn lại (Multi-Trigger + Điều kiện đổi bảng nguồn) đổi StackPanel DataContext=FkLookup + 3 binding `AncestorType=UserControl` chèn `.FkLookup.` (RemoveReloadField/RemoveDataSourceCondition/WhenOpOptions). Root 2.769 → 2.426 dòng; FkLookupConfigVm 500 → 862; XAML −288. Build module 0W/0E (compile XAML)
- [x] B4.2 nhóm 4 (2026-07-18) — Mẫu lookup P4 (SelectedLookupTemplate/LookupTemplates/ParamRows/RebuildTemplateParamRows/BuildParamMapJson/LoadLookupTemplateStateAsync/CanonicalParamDef/NoTemplate) + Cảnh báo cascade (CascadeWarnings/RecomputeCascadeWarnings/regex+token) + Diễn giải (ConfigExplanation/IsExplanationExpanded/ExecuteExplainConfig + 2 command) SỞ HỮU trong FkLookupConfigVm. Coupling khác nhóm 1-3: `RecomputeCascadeWarnings` cần `Navigator.Groups`+`FieldId` (giữ `GetSiblingFieldCodes` ở root, đổi `internal` cho con gọi qua `_root.`) và `ExecuteExplainConfig` cần `IsVirtual` (đọc `_root.IsVirtual`). Reset qua `ResetExplainAndCascadeState` (đúng tập raise cũ, không đụng ReloadOnChangeInput). Root 2.426 → 2.139 dòng; FkLookupConfigVm 862 → 1.156. Build module 0W/0E. **B4.2 xong cả 4 nhóm.**
- [x] B5 (2026-07-18) — `ResetFieldStateForNew` gọi FkLookup lẻ tẻ (ResetComboAndLookupState + ReloadOnChangeInput="" + ResetExplainAndCascadeState) gộp thành **1 method `FkLookup.ResetForNewField()`**; `ClearFkLookupConfig()` GIỮ ở root (cần toggling cờ `_isRebuildingProps` private quanh ResetFkSourceState/ResetLookupDbState — không dời được). Root còn 2 lệnh gọi FkLookup thay vì 4. Root 2.139 → 2.137 dòng; FkLookupConfigVm 1.156 → 1.172. Build module 0W/0E. **5 bước REFACTOR xong toàn bộ — VM root 4.030 → 2.137 dòng (−47%).**
- Smoke thủ công CHƯA CHẠY cho cả 5 bước: mở field TextBox/LookupBox-template/ComboBox → sửa → Lưu → mở lại đối chiếu (navigator, tab Rules/Events, panel LookupBox đủ mục, panel ComboBox, combo Lookup Code RadioGroup)
...
- [~] **AUDIT-1** — Test gần như 0 (5 file test / 358 production; Infrastructure dynamic-SQL không test).
      ...
      **② ĐANG LÀM (hướng a — tách hàm thuần, verify SDK 10 remote):**
        ...
        Infra build 0/0, **test 227/227**. **Còn tách tiếp:** FK JOIN builder (ViewRepository), `DynamicLookupRepository`.
