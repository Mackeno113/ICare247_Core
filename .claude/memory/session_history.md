# Lịch sử session (Archive)

> Nhật ký chi tiết các session **72 → 40** tách khỏi [last_session.md](last_session.md) để giảm
> context mỗi phiên (Nhóm C dọn token, 2026-07-04). Nội dung nguyên văn. `last_session.md` chỉ
> giữ **trạng thái hiện tại**. Việc/roadmap đang mở tra ở [../../TASKS.md](../../TASKS.md).

---

## Session 72 (2026-06-29) — Đánh giá GitNexus (code-intelligence) + license · CHỈ điều tra/quyết định, CHƯA code

> User: "xem xét tài liệu GitNexus, đề xuất phương án tích hợp". Toàn bộ phiên là điều tra + quyết định, KHÔNG sửa
> code sản phẩm. Mọi nhánh đều HỎI trước [[feedback-always-ask-first]]. GitNexus = repo `abhigyanpatwari/GitNexus`
> (npm `gitnexus@1.6.8`) — tool index codebase thành knowledge-graph cho AI agent (impact/context/detect-changes).

### Đã làm
- **Đọc tài liệu GitNexus** (README + LICENSE). Bản chất = developer tool (CLI + MCP), Tree-sitter + LadybugDB; C# = Full Support.
- **Chốt phạm vi với user:** chỉ phân tích **A (tool dev local)** + **B (cảm hứng kiến trúc)**; **loại C** (tính năng sản phẩm).
- **Thử nghiệm A (evaluation 1 máy):** `npx gitnexus@latest analyze` tại repo → **9,336 nodes / 16,837 edges / 299 clusters / 300 flows (26.6s)**; tự loại `bin/obj/_framework` theo `.gitignore`. Chạy thật `query`/`impact "SaveViewAsync"` (bắt 2 symbol trùng interface↔impl)/`detect-changes` (map git-diff dở → 3 execution flow, risk medium, đúng `GetDataAsync` trong ViewRepository.cs). CLI có: query/context/impact/trace/cypher/detect-changes.
- **Phân tích license PolyForm Noncommercial 1.0.0:** "noncommercial" đo bằng **MỤC ĐÍCH**, không phải doanh thu; solo/pre-revenue **không** tự thành noncommercial nếu có "anticipated commercial application". Ở mode A tool **không nhúng** sản phẩm → IP sản phẩm **không bị nhiễm**; "xóa trước khi bán" không hồi tố được hành vi dùng. Rủi ro thực tế (solo, local, không phân phối) **thấp**, nhưng không "sạch" về lý.
- **Soạn email hỏi license** (EN + VI) → **user ĐÃ GỬI** tới `abhigyan1.patwari@gmail.com` (maintainer duy nhất, Guwahati IN). Hỏi: solo/pre-revenue local-dev có nằm trong noncommercial? điều khoản commercial cho 1 dev? mốc phải chuyển license?

### ⚠️ Artefact GitNexus còn trên đĩa (CHƯA dọn — chờ user quyết)
- `.gitnexus/` (≈98 MB, file `lbug`) — **tự ignore** (có `.gitignore=*` bên trong) → không commit nhầm. Xóa = `npx gitnexus clean`.
- `.claude/skills/gitnexus/` (6 SKILL.md) — **ĐANG UNTRACKED** → `git add .` sẽ tóm. **KHÔNG commit khi license chưa rõ.**
- Không đụng `settings.json`/MCP config.

### ⏳ Việc cần làm (để ngỏ)
1. **Chặn commit nhầm (an toàn, không dính license):** thêm vào `.gitignore`:
   `.gitnexus/` và `.claude/skills/gitnexus/`. — CHƯA làm (user nói "không làm gì thêm").
2. **Chờ hồi âm tác giả** về license → rẽ nhánh:
   - OK/rẻ cho solo → nâng A dùng chính thức (`gitnexus setup` + đăng ký MCP, có thể commit skills).
   - Chỉ commercial đắt → bỏ A, dồn vào B.
3. **Phương án B (đường sạch, KHÔNG phụ thuộc license, GIÁ TRỊ SẢN PHẨM):** thiết kế impact-engine nội bộ
   trên `Sys_Dependency` (đã có `IDependencyRepository`/`DependencyRepository` + WPF `DependencyViewerView`) — mượn
   *ý tưởng* precompute-impact/blast-radius/detect-changes, áp xuống tầng **config** (đổi field/column/rule → cảnh báo
   form/validation/event ảnh hưởng; nối ConfigSync master→tenant). Clean-room: KHÔNG copy code/port thuật toán GitNexus.
4. **Khi tới mốc** (người thứ 2 dùng / vào CI / có doanh thu) → bắt buộc có commercial license hoặc dừng dùng A.

### Quyết định / ghi nhớ
- Phân tích A vs B vs C + đánh giá thực nghiệm A + phân tích license = **chưa sửa 1 dòng code sản phẩm nào**; mọi file `.M`
  trong git status là tồn từ session 70/71 (3 file i18n + WPF FK-lookup), **không thuộc** session này.
- Hướng đầu tư đúng chất ICare247 (no-code, sở hữu, không vướng license) = **B**, không phải phụ thuộc GitNexus.
...
> User: "đọc luồng cấu hình WPF, tìm điểm nghẽn mất thời gian / mất điểm UX / lặp thao tác" → phân tích →
> "đề xuất phương án tối ưu, mục tiêu **hạn chế thao tác nhấn nút**". User chốt qua hỏi-đáp:
> **mô hình lưu đích = auto-save hoàn toàn**; **bắt đầu = cache i18n + bỏ nút Generate** → làm cả 1A+1B.

### Phân tích điểm nghẽn (đã trình bày user)
- **N+1 query i18n mỗi lần mở form**: `FormEditorViewModel.LoadFromDatabaseAsync` resolve `ResolveKeyAsync` RIÊNG
  cho từng tab/section/field (tuần tự, mỗi lần mở connection mới) — form 50 field ≈ 56 round-trip.
- **5 cơ chế lưu song song** (SaveForm/_autoSave/_fieldQuickSave/SaveSection/SaveTab/FieldConfig.SaveField) → user mơ hồ "đã lưu chưa".
- **FieldConfig là màn navigation riêng** → cấu hình sâu phải Open/Back + full-reload form mỗi field.
- **i18n key thủ công** (4 key/field, mỗi key 1 nút Generate + resolve preview) dù key deterministic.
- **Nguồn FK gõ tay** (FkTableName/ValueField/DisplayField) không dropdown từ schema.

### Phase 1 — ĐÃ CODE (1A + 1B)
- **1A cache i18n** — `I18nDataService`: cache `ConcurrentDictionary` nạp toàn bộ Sys_Resource 1 query
  (`EnsureLoadedAsync`, key `"{key}|{lang}"`), `ResolveKeyAsync` đọc RAM; write-through ở Save/Init/Delete; tự reload
  khi đổi connection string; thêm `RefreshAsync`. DI **`Register`→`RegisterSingleton`** (App.xaml.cs) — N+1 → 1 query.
- **1B bỏ nút Generate** — key tự sinh ngầm theo cột (`AutoDeriveI18nKeys` từ setter SelectedColumn/FieldCode/IsVirtual,
  guard `_isLoading`); 4 ô Label/Placeholder/Tooltip/Required → **nhập tiếng Việt thẳng** (key thành caption mờ);
  Lưu field upsert bản dịch (`UpsertOrInitViAsync`); chống dirty oan (`_suppressValueDirty`) + giữ text user nếu chưa
  có bản dịch. Xoá 4 `Generate*KeyCommand` + 2 method + nút XAML. Giữ "🌐 Dịch".
- **Files**: `Infrastructure/I18nDataService.cs` · `Core/Interfaces/II18nDataService.cs` · `App.xaml.cs` ·
  `Modules.Forms/ViewModels/FieldConfigViewModel.cs` · `Modules.Forms/Views/FieldConfigView.xaml`. Build 0/0.

### Quyết định (LUẬT)
- **i18n resolve = cache-first, singleton** — không bao giờ N+1 ResolveKeyAsync trong vòng lặp; cache invalidate qua
  write-through + đổi-connection-string. Mọi mutation qua II18nDataService giữ cache đồng nhất.
- **Mô hình lưu đích = auto-save toàn cục** (user chốt) — Phase 2 sẽ gộp 5 cơ chế lưu thành 1 + chỉ báo trạng thái.
- **Key i18n deterministic = tự sinh, KHÔNG bắt user bấm nút** — user gõ giá trị, không gõ key.

### ⏳ Còn lại / tiếp theo
- **Verify thực tế** (chưa chạy WPF app): mở form lớn đo độ trễ; chọn cột→nhãn VI tự nạp; gõ→Lưu→mở lại; "🌐 Dịch".
- **Phase 2** = auto-save toàn cục. Phase sau: bỏ full-reload sau save field (patch node); inline field config (bỏ Open/Back);
  dropdown FK từ schema (GetColumnsByTableAsync); debounce RebuildControlPropsJson.

---

> Cập nhật: 2026-06-26 (session 65 — WPF Form Editor: fix "Số cột không lưu" + auto-fill từ Sys_Table)

## Session 65 (2026-06-26) — đã làm (ĐÃ CODE; build WPF Forms module 0/0)

> User phản hồi màn Form Editor (ConfigStudio): nhập lại quá nhiều thông tin đã có ở Sys_Table + "Số cột" không lưu.

- **FIX bug "Số cột không lưu"**: ComboBox `EditValue="{Binding FormColumns}"` thiếu `UpdateSourceTrigger=PropertyChanged`
  → DXEditors commit EditValue lúc **LostFocus**; nút Lưu toolbar (Focusable=False) không làm combo mất focus → giá trị
  không ghi về VM → lưu cũ/null. Sửa: thêm `Mode=TwoWay, UpdateSourceTrigger=PropertyChanged` (FormEditorView.xaml).
  Đường lưu DB vốn ĐÚNG (`UpdateFormMetadataAsync` set `Form_Columns`). Gotcha chung mọi DXEditor binding EditValue.
- **UX auto-fill**: chọn Business Table → `AutoFillFromTable` tự điền Form Code (=Table_Code uppercase) + Tên Form
  (=Table_Name) từ Sys_Table. CHỈ khi tạo mới + ô trống (không đè giá trị user). Giảm: pick table → 1 click Tạo Form.

- **Rà + vá loạt (user duyệt "sửa loạt")**: 101 binding `EditValue` toàn ConfigStudio → vá **30 combo/value thiếu
  `UpdateSourceTrigger`** (FormEditor 4, ValidationRuleEditor 3, RelationManager 2, ExpressionBuilder 1, FieldConfig 8,
  ViewManager 6, LookupBoxProps 2, ComboBoxProps 4). Build toàn slnx 0/0. Combo LỌC (FormManager/I18n/Grammar) bỏ qua.

- **Tối ưu Auto-generate Fields (UX "ít thao tác")**: trước đã suy EditorType (theo kiểu) + IsRequired (NOT NULL).
  Thêm **FK auto → LookupBox + Ui_Field_Lookup**. **NGUỒN QUAN HỆ CHÍNH = `Sys_Relation`** (user chốt: "relationship
  lấy ở đây là chính") — KHÔNG cần FK vật lý: PersistAutoGeneratedFieldsAsync nạp `IRelationDataService.GetRelationsAsync`
  lọc Detail_Table_Id=bảng form → map Detail_FK_Column→relation; cột trùng → LookupBox + LookupSource="dynamic" +
  FieldLookupConfigRecord (Source=MasterTableCode, Value=ValueColumn/MasterKeyColumn, Display=DisplayColumn — tường minh).
  **Fallback** = FK vật lý DB (SchemaInspector đọc REFERENTIAL_CONSTRAINTS → ColumnSchemaDto.IsForeignKey). IRelationDataService
  inject vào FormEditorViewModel (đã đăng ký App.xaml.cs:69). KHÔNG cần migration. Compile CS 0 lỗi (slnx fail chỉ do copy-lock).
  Backlog: auto label tiếng Việt + auto Unique + cascade tự động.

### Quyết định (Sys_Relation là hub quan hệ) — LUẬT
- **Quan hệ lookup lấy từ `Sys_Relation` (Config DB) là DUY NHẤT** (user chốt LUẬT 2026-06-26: "WPF chuyển TẤT CẢ logic
  khóa ngoại về Sys_Relation"). **ĐÃ GỠ** logic FK vật lý DB (SchemaInspector REFERENTIAL_CONSTRAINTS + ColumnSchemaDto
  FK fields + nhánh fallback). Lý do no-code: user cấu hình ở RelationManager, có sẵn Display/Value/FK column tường minh.
- **LUẬT thao tác (user nhấn mạnh)**: KHÔNG tự quyết định LOGIC trước khi hỏi — kể cả "fallback an toàn". Xem [[feedback-always-ask-first]].
- `RelationRecord`: DetailTableId+DetailFkColumn (bảng con + cột FK) → MasterTableCode (nguồn) + ValueColumn/MasterKeyColumn
  + DisplayColumn. Gotcha: `DM_ChiNhanhNganHang` thiếu PK → nếu lookup Value=Id cần bảng có Id ổn định (nên vá — backlog).

### Gotcha (lưu ý cho WPF)
- **Mọi DXEditor (ComboBoxEdit/TextEdit...) phải thêm `UpdateSourceTrigger=PropertyChanged` vào EditValue** nếu muốn
  commit ngay khi đổi (không đợi LostFocus) — nếu không, lưu qua nút toolbar Focusable=False sẽ mất giá trị vừa chọn.
- **Build WPF khi ConfigStudio đang chạy = MSB3021/3026/3027 copy-lock** (KHÔNG phải lỗi CS) → đóng app rồi rebuild.
  Verify compile: build riêng project (`...Modules.Forms.csproj`) hoặc grep `error CS` (bỏ qua MSB copy).

---

> Cập nhật: 2026-06-26 (session 64 — sửa màn Công ty theo schema thật + module upload logo DB+BE)

## Session 64 (2026-06-26) — đã làm (ĐÃ CODE BE; build Api 0/0)

> User chỉ ra guide màn Công ty (session 63) làm ẩu + schema DB đã đổi. Sửa lại theo DDL thật user cung cấp +
> dựng module upload logo (DB + backend trước, FE đợt sau).

### Schema thật (user dán DDL) — đã nắm
- `TC_CongTy`: **bỏ `NganHang_Id`**, thêm `ChiNhanhNganHang_Id`/`Logo_Id`/`NgayThanhLap`.
- `vw_TC_CongTy`: JOIN `DM_ChiNhanhNganHang→DM_NganHang`; expose `TinhThanhPho_Id`/`TenTinhThanhPho` (đủ cascade).
- `DM_ChiNhanhNganHang` (mới, user tạo) **thiếu chuẩn**: `Id int NULL`, KHÔNG PK/IDENTITY/FK/audit → cần vá (ADR-022).

### Sửa nhận thức màn Công ty (chưa sửa guide — TODO CONGTY-GUIDE-FIX)
- Công ty cha = **TreePicker** (`treelookup`), KHÔNG LookupBox phẳng.
- **Tỉnh ảo → Phường/Xã** cascade (`Reload_Trigger_Field`/`Parent_Column`=TinhThanhPho_Id) — DM_PhuongXa CÓ TinhThanhPho_Id.
- **Ngân hàng ảo → ChiNhanhNganHang** cascade; bỏ field ngân hàng trực tiếp.
- Logo = editor **ImageUpload**; thêm NgayThanhLap.

### Module upload logo (storage A = bytes trong DB) — ĐÃ CODE DB + BE
- **db/063** `TT_TepDinhKem` (Data DB, đúng chuẩn audit) + FK `TC_CongTy.Logo_Id`. Cột `Storage_Kind`/`Storage_Key`
  để sau cắm FileSystem(đường dẫn TƯƠNG ĐỐI)/Object storage. ⏳ CẦN chạy Data DB.
- **BE**: `FilesController` `POST /api/v1/files` (multipart, validate MIME png/jpeg/webp + **magic-byte** + ≤2MB,
  sha256) + `GET /api/v1/files/{id}` (stream inline + ETag + 304). CQRS Upload/GetFile + `IFileAttachmentRepository`
  /`FileAttachmentRepository` (VARBINARY bind DbType.Binary size=-1) + DI. Build Api 0/0.
- **FE (chưa làm)**: editor `ImageUpload` → renderer (InputFile→POST→set Logo_Id; preview qua HttpClient+objectURL
  vì serve cần Bearer) + Ui_Control_Map + normalizer.

### Quyết định
- **Lưu logo = bytes trong DB (A)**, KHÔNG đường dẫn tuyệt đối (vỡ đa máy + path traversal). [[project-config-sync-engine]]? không — ghi memory module file riêng nếu cần.
- **Cách tôi đọc DB**: live sqlcmd `localhost` fail (Named Pipes); cách dùng = **user dán DDL** (đang chạy tốt) hoặc bật TCP + cho tên server.
- IsDeleted/audit BẮT BUỘC ở MỌI bảng nghiệp vụ Data DB (ADR-022) — TT_TepDinhKem tuân thủ; DM_ChiNhanhNganHang chưa.

### ⏳ Còn lại
- Chạy **db/063** (Data DB). FE editor ImageUpload (FILE-FE). Sửa guide màn Công ty (CONGTY-GUIDE-FIX). Vá DM_ChiNhanhNganHang.

---

> Cập nhật: 2026-06-25 (session 63 — VFILTER-ACTIVE: company-switcher full-stack + CC-4 invalidate cache sau sync)

## Session 63 (2026-06-25) — đã làm (ĐÃ CODE; build BE Api 0/0 + FE 0/0)

> Tiếp sau F1: đóng nốt CC-4 rồi làm VFILTER-ACTIVE (company-switcher) theo lựa chọn user.

- **CC-4** — `SyncConfigCommandHandler` bump `ICacheVersion` sau apply thành công → vô hiệu toàn bộ cache config
  dùng chung tenant (form/view/lookup/resource) → thấy cấu hình mới ngay sau sync, không cần restart.
- **VFILTER-ACTIVE — company-switcher full-stack (ADR-030):**
  - BE: `GET /api/v1/me/companies` — `GetMyCompaniesQuery`/Handler + `IMeCompanyRepository`/`MeCompanyRepository`
    (Data DB): JOIN `HT_NguoiDung_CongTy × TC_CongTy` theo `@NguoiDungID`; **fallback** mọi công ty active nếu bảng
    chưa có (OBJECT_ID guard) hoặc user chưa được phân công; `LaMacDinh` lên đầu. Endpoint ở `MeController` [Authorize].
  - FE: `AppState` refactor (`ActiveCompanyId` **long** + `Companies` + record `CompanyOption`); `ActiveScopeHandler`
    (DelegatingHandler NGOÀI CÙNG, đính header `X-Active-CongTy` từ AppState — chuỗi ActiveScope→RefreshToken→Http);
    `MeCompanyApiService`; `CompanySwitcher.razor` (topbar, native select, localStorage `ic247.activeCongTy`, đổi →
    reload trang); MainLayout nạp SỚM id đã lưu (tránh race header) + Program.cs wiring + DI.
  - Chọn "Tất cả công ty" (null) → bỏ header → server default `@CongTyID_Active`=0. Ranh giới bảo mật = `@NguoiDungID`
    (server validate `@CongTyID_Active` độc lập nên switcher liệt kê công ty nào cũng không hở).

### Phát hiện
- `HT_NguoiDung_CongTy` là **bảng THẬT** (db/037: NguoiDung_Id/CongTy_Id/LaMacDinh/IsDeleted), không chỉ "mẫu" —
  khớp Validate_Sql của token CongTyID_Active. VFILTER-OPEN chỉ còn: seed dữ liệu phân công thật.
- **Provisioning hook (F1) BỊ CHẶN**: chưa có luồng tạo tenant (catalog fallback). `SyncConfigCommand` đã là full-sync.

### ⏳ Còn lại (S63)
- E2E company-switcher: cần `TC_CongTy` có dữ liệu + login → đổi công ty → kiểm lưới lọc theo `@CongTyID_Active`.
- (tùy) cho lưới tự reload khi đổi công ty thay vì forceLoad reload (subscribe AppState.OnChange ở ViewPage).
- CTXPARAM-3 (WPF Sys_Context_Param CRUD); VFILTER-OPEN seed phân công.
...
### CC-4 — ĐÃ CODE (build Application 0/0)
- `SyncConfigCommandHandler` bump `ICacheVersion` sau khi apply thành công → vô hiệu TOÀN BỘ cache config dùng chung
  (form/view/lookup/resource) của tenant → thấy cấu hình mới NGAY, không cần restart API (trước chỉ invalidate menu).
...
- ⏸️ **Provisioning full-sync BỊ CHẶN**: chưa có luồng tạo tenant (catalog chưa dựng — `TenantConnectionResolver`
  chạy fallback 1-tenant). `SyncConfigCommand(DryRun=false)` ĐÃ là full-sync → khi dựng subsystem provisioning chỉ cần
  gọi nó cho tenant mới. Hoãn tới khi có catalog + endpoint tạo tenant.
- Sau F1 → **F2 engine-hóa màn Công ty** (ORG-CFG-3 + CAT-CFG-2 = cấu hình tay ConfigStudio WPF, no-code).
